...
def main(args):
    ...
    parser.add_argument(
        "-w", "--weights", nargs="+", help="Gene predictors and weights"
    )
    ...
    parser.add_argument(
        ...
        help="Specify species for Augustus",
    )
    parser.add_argument(
        "--genemark_mod",
    ...
    parser.add_argument(
        "--transcript_alignments", help="Transcript evidence in GFF3 format"
    )
    parser.add_argument(
        "-gm",
        "--genemark_mode",
        default="ES",
        choices=["ES", "ET"],
        help="Mode to run genemark in",
    )
    parser.add_argument(
        "--pasa_gff", help="Pre-computed PASA/TransDecoder high quality models"
    )
    parser.add_argument(
        "--other_gff", nargs="+", help="GFF gene prediction pass-through to EVM"
    )
    parser.add_argument(
        "--augustus_gff", help="Pre-computed Augustus gene models (GFF3)"
    )
    parser.add_argument(
        "--genemark_gtf", help="Pre-computed GeneMark gene models (GTF)"
    )
    parser.add_argument(
        "--soft_mask",
        type=int,
    ...
    parser.add_argument(
        "--repeats2evm", action="store_true", help="Pass repeat GFF3 to EVM"
    )
    parser.add_argument(
        "--repeat_filter",
        default=["overlap", "blast"],
        nargs="+",
        choices=["overlap", "blast", "none"],
    ...
    parser.add_argument(
        ...
        default=10,
        type=int,
        help="Minimum intron length for gene models",
    )
    parser.add_argument(
        "--max_intronlen",
        default=3000,
    ...
    parser.add_argument(
        "--SeqCenter", default="CFMR", help="Sequencing center for GenBank tbl file"
    )
    parser.add_argument(
        "--SeqAccession", default="12345", help="Sequencing accession number"
    )
    parser.add_argument(
    ...
    if not os.path.isdir(args.out):
        ...
        dirs = [
            ...
            os.path.join(args.out, "predict_results"),
        ]
        for d in dirs:
    ...
    if args.out in conflict:
        lib.log.error(
            "%s output folder conflicts with a hard coded tmp folder, please change -o parameter"
            % args.out
        )
        sys.exit(1)
    ...
    if not os.path.isfile(blastdb):
        lib.log.error(
            "Can't find Repeat Database at {:}, you may need to re-run funannotate setup".format(
    ...
    if args.AUGUSTUS_CONFIG_PATH:
        ...
        try:
            ...
            lib.log.error(
                "$AUGUSTUS_CONFIG_PATH environmental variable not found, Augustus is not properly configured. You can use the --AUGUSTUS_CONFIG_PATH argument to specify a path at runtime."
            )
            sys.exit(1)
    ...
    if lib.which_path("bam2hints"):
        ...
        if not lib.which_path(BAM2HINTS):
    ...
    if lib.which_path("gff2gbSmallDNA.pl"):
        GFF2GB = "gff2gbSmallDNA.pl"
    else:
        GFF2GB = os.path.join(AUGUSTUS_BASE, "scripts", "gff2gbSmallDNA.pl")
        if not lib.which_path(GFF2GB):
            scripts_missing.append(GFF2GB)
    if len(scripts_missing) > 0:
        lib.log.error(
            "ERROR: unable to locate the following Augustus scripts:\n{}".format(
                "\n".join(scripts_missing)
            )
        )
        lib.log.error(
            "Either add these scripts to your PATH or ensure that $AUGUSTUS_CONFIG_PATH/../scripts/ exists."
        )
        sys.exit(1)
    ...
    if GENEMARK_PATH:
        try:
            ...
            lib.log.debug(
                "GeneMark appears to be functional? {:}".format(genemarkcheck)
            )
        except NameError:
            GENEMARKCMD = ""
            genemarkcheck = False
    ...
    if os.path.isdir(os.path.join(args.out, "training")):
        ...
        if os.path.isfile(os.path.join(traindir, "funannotate_train.coordSorted.bam")):
            if not args.rna_bam:
                ...
                pre_existing.append(
                    "  --rna_bam "
                    + os.path.join(traindir, "funannotate_train.coordSorted.bam")
                )
        if os.path.isfile(os.path.join(traindir, "funannotate_train.pasa.gff3")):
            if not args.pasa_gff:
                ...
                pre_existing.append(
                    "  --pasa_gff "
                    + os.path.join(traindir, "funannotate_train.pasa.gff3")
                )
        if os.path.isfile(os.path.join(traindir, "funannotate_train.stringtie.gtf")):
            if not args.stringtie:
                ...
                pre_existing.append(
                    "  --stringtie "
                    + os.path.join(traindir, "funannotate_train.stringtie.gtf")
                )
        if os.path.isfile(os.path.join(traindir, "funannotate_train.transcripts.gff3")):
            if not args.transcript_alignments:
                ...
                pre_existing.append(
                    ...
                    + os.path.join(traindir, "funannotate_train.transcripts.gff3")
                )
        else:
            if os.path.isfile(
                os.path.join(traindir, "funannotate_train.trinity-GG.fasta")
            ):
                if not args.transcript_evidence:
                    ...
                    if (
                        not os.path.join(traindir, "funannotate_train.trinity-GG.fasta")
                        in args.transcript_evidence
                    ):
                        args.transcript_evidence.append(
                            os.path.join(traindir, "funannotate_train.trinity-GG.fasta")
                        )
                        pre_existing.append(
                            "  --transcript_evidence "
                            + " ".join(args.transcript_evidence)
                        )
            if os.path.isfile(os.path.join(traindir, "funannotate_long-reads.fasta")):
                if not args.transcript_evidence:
                    args.transcript_evidence = [
                        os.path.join(traindir, "funannotate_long-reads.fasta")
                    ]
                    pre_existing.append(
                        "  --transcript_evidence "
                        + os.path.join(traindir, "funannotate_long-reads.fasta")
                    ...
                    if (
                        not os.path.join(traindir, "funannotate_long-reads.fasta")
                        in args.transcript_evidence
                    ):
                        args.transcript_evidence.append(
                            os.path.join(traindir, "funannotate_long-reads.fasta")
    ...
    if len(pre_existing) > 0:
        lib.log.info(
            "Found training files, will re-use these files:\n%s"
            % "\n".join(pre_existing)
        )
    ...
    lib.copyDirectory(
        os.path.join(FUNDB, "trained_species", args.busco_seed_species),
        os.path.join(LOCALAUGUSTUS, "species", args.busco_seed_species),
        overwrite=True,
    )
    lib.copyDirectory(
        os.path.join(FUNDB, "trained_species", "anidulans"),
        os.path.join(LOCALAUGUSTUS, "species", "anidulans"),
        overwrite=True,
    )
    lib.copyDirectory(
        os.path.join(AUGUSTUS, "species", "generic"),
        os.path.join(LOCALAUGUSTUS, "species", "generic"),
        overwrite=True,
    )
    lib.copyDirectory(
        os.path.join(AUGUSTUS, "extrinsic"),
        os.path.join(LOCALAUGUSTUS, "extrinsic"),
        overwrite=True,
    )
    lib.copyDirectory(
        os.path.join(AUGUSTUS, "model"),
        os.path.join(LOCALAUGUSTUS, "model"),
        overwrite=True,
    )
    lib.copyDirectory(
        os.path.join(AUGUSTUS, "profile"),
        os.path.join(LOCALAUGUSTUS, "profile"),
        overwrite=True,
    )
    if os.path.isdir(os.path.join(AUGUSTUS, "parameters")):
        lib.copyDirectory(
            os.path.join(AUGUSTUS, "parameters"),
            os.path.join(LOCALAUGUSTUS, "parameters"),
            overwrite=True,
        )
    ...
    if args.parameters:
        ...
        if augspeciescheck:
            ...
            with open(
                os.path.join(FUNDB, "trained_species", aug_species, "info.json")
            ) as infile:
    ...
    if "path" in trainingData["augustus"][0]:
        ...
        if args.pasa_gff:
            ...
            if not os.path.isdir(
                ...
                lib.log.error(
                    "ERROR: --busco_seed_species {} is not valid as it is not in database. Try `funannotate species` command and check spelling."
                )
                sys.exit(1)
            sourceText = "BUCSCO {:}".format(args.busco_db)
        trainingData["augustus"] = [
            {
                "version": "funannotate v{:}".format(version),
                "source": sourceText,
                "date": TODAY,
    ...
    if genemarkcheck:
        if "path" in trainingData["genemark"][0]:
            ...
            shutil.copyfile(
                trainingData["genemark"][0]["path"],
                os.path.join(LOCALPARAMETERS, aug_species + ".genemark.mod"),
            )
        ...
        trainingData["glimmerhmm"] = [
            {
                ...
                "source": sourceText,
                "date": TODAY,
                "path": os.path.abspath(os.path.join(LOCALPARAMETERS, "glimmerhmm")),
            }
        ]
    if StartWeights["codingquarry"] > 0:
        if "path" in trainingData["codingquarry"][0] and "QUARRY_PATH" in os.environ:
            ...
            lib.copyDirectory(
                os.environ["QUARRY_PATH"],
                os.path.join(args.out, "predict_misc", "CodingQuarry", "QuarryFiles"),
                overwrite=True,
            )
            lib.copyDirectory(
                trainingData["codingquarry"][0]["path"],
                os.path.join(
                    args.out,
                    "predict_misc",
                    "CodingQuarry",
            ...
            if args.rna_bam and "QUARRY_PATH" in os.environ:
                lib.copyDirectory(
                    os.environ["QUARRY_PATH"],
                    os.path.join(
                        args.out, "predict_misc", "CodingQuarry", "QuarryFiles"
                    ),
                    overwrite=True,
                )
                ...
                trainingData["codingquarry"] = [
                    {
                        ...
                        "date": TODAY,
                        "path": os.path.abspath(
                            os.path.join(LOCALPARAMETERS, "codingquarry")
                        ),
                    }
                ]
    elif args.rna_bam and "QUARRY_PATH" in os.environ:
        lib.log.info(
            "Skipping CodingQuarry as --organism=other. Pass a weight larger than 0 to run CQ, ie --weights codingquarry:1"
        )
    ...
    if (
        ...
        and "codingquarry" not in RunModes
        and StartWeights["codingquarry"] > 0
    ):
    ...
    if not augustus_functional and RunBusco:
        ...
        if "MacOSX" in system_os:
            lib.log.error(
                "OS={:}, try to install augustus v3.2.1 from https://github.com/nextgenusfs/augustus or from HomeBrew".format(
                    system_os
                )
            )
        sys.exit(1)
    ...
    if args.rna_bam:
        ...
        if not bam2hints_functional:
            lib.log.error(
                "ERROR: {:} is not installed properly check compilation".format(
                    BAM2HINTS
                )
            )
            sys.exit(1)
    ...
    if args.pasa_gff:
        ...
        if not lib.evmGFFvalidate(PASA_GFF, EVM, lib.log):
            lib.log.error(
                "ERROR: %s is not a properly formatted PASA GFF file, please consult EvidenceModeler docs"
                % args.pasa_gff
            )
            sys.exit(1)
    ...
    if len(other_files) > 0:
        for i, file in enumerate(other_files):
            ...
            lib.log.info(
                "Parsing GFF pass-through: {:} --> setting source to {:}".format(
                    file, featurename
                )
            )
            ...
            if not lib.evmGFFvalidate(outputGFF, EVM, lib.log):
                lib.log.error(
                    "ERROR: {} is not proper GFF file, please consult EvidenceModeler docs".format(
                        file
                    )
                )
                sys.exit(1)
    ...
    if args.input:
        ...
        if len(bad_headers) > 0:
            ...
            lib.log.error(
                "Found {:,} contigs contain non-IUPAC characters:".format(
                    len(bad_contigs)
                )
            )
        ...
        if maskedSize == 0 and not args.force:
            ...
            lib.log.info(
                "Genome loaded: {:,} scaffolds; {:,} bp; {:.2%} repeats masked".format(
        ...
        if len(other_contigs) > 0:
            ...
            if len(extra_contigs) > 0:
                lib.log.error(
                    "ERROR: found {:,} contigs in --other_gff that are not found in the genome assembly.".format(
                        len(extra_contigs)
                    )
                )
                lib.log.error(
                    "  --other_gff should contain gene models from. Sample of offending contigs: {}\n  {}".format(
                        args.input, ", ".join(extra_contigs[:10])
                    )
                )
                sys.exit(1)
    ...
    AUGUSTUS_PARALELL = os.path.join(parentdir, "aux_scripts", "augustus_parallel.py")
    ...
    if args.maker_gff:
        ...
        if args.transcript_alignments:
            lib.harmonize_transcripts(
                MaskGenome,
                args.transcript_alignments,
                trans_out,
                hintsM,
                evidence=args.transcript_evidence,
                tmpdir=os.path.join(args.out, "predict_misc"),
                cpus=args.cpus,
            ...
            if args.transcript_evidence:
                ...
                if "minimap2" in args.aligners:
                    ...
                    if not lib.checkannotations(
                        minimapGFF3
                    ) or not lib.checkannotations(hintsM):
                        lib.log.info(
                        ...
                        lib.log.info(
                            "Found {:,} alignments, wrote GFF3 and Augustus hints to file".format(
                                minimapCount
                            )
                        )
                    else:
                        lib.log.info(
                            "Existing minimap2 alignments found: {:} and {:}".format(
                                minimapGFF3, hintsM
                            )
                        )
                if "gmap" in args.aligners:
                    ...
                    if not lib.checkannotations(gmapGFF3):
                        ...
                        lib.log.info(
                            "Found {:,} alignments, wrote GFF3 to file".format(
                                gmapCount
                            )
                        )
                    else:
                ...
                if "blat" in args.aligners:
                    if not lib.checkannotations(hintsE):
                        ...
                        lib.log.info(
                            "{0:,}".format(total) + " filtered BLAT alignments"
                        )
                    else:
        ...
        if args.rna_bam:
            if not lib.checkannotations(hintsBAM):
                ...
                lib.runSubprocess(
                    ...
                    ".",
                    lib.log,
                    in_file=bamhintssorted,
                    capture_output=bamjoinedhints,
                )
                # filter intron hints
                cmd = [
                    os.path.join(
                        parentdir, "aux_scripts", "filterIntronsFindStrand.pl"
                    ),
                    MaskGenome,
                    bamjoinedhints,
                    "--score",
        ...
        if not args.exonerate_proteins:
            if args.protein_evidence:
                ...
                p2g_cmd = [
                    ...
                    args.tmpdir,
                    "--tblastn_out",
                    os.path.join(args.out, "predict_misc", "p2g.diamond.out"),
                    "--logfile",
                    os.path.join(args.out, "logfiles", "funannotate-p2g.log"),
                ]
            ...
            lib.log.info(
                "Loading protein alignments {:}".format(args.exonerate_proteins)
            )
        ...
        if (
            lib.checkannotations(hintsP)
            or lib.checkannotations(hintsE)
            or lib.checkannotations(hintsBAM)
            or lib.checkannotations(hintsM)
        ):
        ...
        if lib.checkannotations(allhintstmp):
            ...
            cmd = [JOINHINTS]
            lib.runSubprocess(
                cmd, ".", lib.log, in_file=allhintstmp_sort, capture_output=hints_all
            )
...
-------------------------------------------------------"
            ...
            if GeneMark:
                ...
                if len(Contigsmissing) > 0:
                    ...
                    for x in fileList:
                        cmd = [
                            os.path.join(GENEMARK_PATH, "hmm_to_gtf.pl"),
                            "--in",
                            x,
                            "--app",
                            "--out",
                    ...
                    cmd = [
                        ...
                        "--trace",
                        os.path.join(
                            args.out, "predict_misc", "genemark", "info", "dna.trace"
                        ),
                        "--in",
                        genemarkGTFtmp,
                        "--back",
                    ...
                    with open(GeneMark, "w") as output:
                        with open(GeneMarkTemp, "r") as input:
                            ...
                            output.write(lines)
                lib.log.info(
                    "{:,} predictions from GeneMark".format(lib.countGFFgenes(GeneMark))
                )
                if os.path.isfile(os.path.join(args.out, "predict_misc", "gmhmm.mod")):
                    shutil.copyfile(
                        os.path.join(args.out, "predict_misc", "gmhmm.mod"),
                        os.path.join(LOCALPARAMETERS, aug_species + ".genemark.mod"),
                    )
                else:
                    lib.log.debug(
        ...
        if RunBusco:
            ...
            if not lib.checkannotations(busco_final):
                if sys.version_info > (3, 0):
                    ...
                    BUSCO = os.path.join(
                        parentdir, "aux_scripts", "funannotate-BUSCO2-py2.py"
                    )
                ...
                if runbuscocheck:
                    ...
                    if tblastn_version > "2.2.31":
                        lib.log.info(
                            "Multi-threading in tblastn v{:} is unstable, running in single threaded mode for BUSCO".format(
                                tblastn_version
                            )
                        )
                ...
                lib.log.info(
                    "{:,} valid BUSCO predictions found, validating protein sequences".format(
                        len(busco_complete)
                    )
                )
                ...
                with open(busco_augustus_tmp, "w") as output:
                    for i in busco_complete:
                        ...
                        subprocess.call(
                            ["perl", Converter2, file], stderr=FNULL, stdout=output
                        )
                # finally rename models so they are not redundant
                busco_augustus = os.path.join(
                    args.out, "predict_misc", "busco_augustus.gff3"
                )
                busco_proteins = os.path.join(
                    args.out, "predict_misc", "busco_augustus.proteins.fasta"
                )
                ...
                with open(busco_log, "a") as logfile:
                    subprocess.call(
                        [
                            ...
                            BUSCO,
                            "-i",
                            os.path.abspath(busco_proteins),
                            "-m",
                            "proteins",
                            "--lineage",
                            BUSCO_FUNGI,
                ...
                lib.filterGFF3(
                    buscoProtComplete, MaskGenome, busco_augustus, busco_final
                )
            ...
            cmd = [
                FILTERGENE,
                os.path.abspath(trainingModels),
            ...
            if not AllPretrained:
                lib.log.error(
                    "RunBusco is set to False and args.pasa_gff is None --> cannot generate training set. If you are reading this it is a bug, please report."
                )
                sys.exit(1)
        ######################
        #     Augustus       #
            ...
            if not lib.checkannotations(aug_out):
                ...
                if RunModes["augustus"] != "pretrained":
                    ...
                    if totalTrain > 1000:
                        ...
                        numTrainingSet = 100
                    lib.log.info(
                        "Training Augustus using {:} gene models".format(
                    ...
                    trainingset = os.path.join(
                        args.out,
                        "predict_misc",
                        "augustus.training.{:}.gb".format(RunModes["augustus"]),
                    )
                    ...
                    lib.trainAugustus(
                        ...
                        args.cpus,
                        numTrainingSet,
                        args.optimize_augustus,
                        os.path.abspath(LOCALAUGUSTUS),
                    )

                # now run Augustus multithreaded...
                lib.log.info(
                    "Running Augustus gene prediction using {:} parameters".format(
                        aug_species
                    )
                )
                if os.path.isfile(hints_all):
                    cmd = [
                        ...
                        str(args.cpus),
                        "-e",
                        os.path.join(parentdir, "config", "extrinsic.E.XNT.RM.cfg"),
                        "--logfile",
                        os.path.join(args.out, "logfiles", "augustus-parallel.log"),
                    ]
                else:
                    cmd = [
                        AUGUSTUS_PARALELL,
                    ...
                    for line in pred:
                        ...
                        if not args.rna_bam:
                            ...
                            if line.startswith("# CDS introns:"):
                                ...
                                try:
                                    support = (
                                        int(intronMatch.split("/")[0])
                                        / float(intronMatch.split("/")[1])
                                        * 100
                                    )
        ...
        if "codingquarry" in RunModes:
            if not lib.checkannotations(Quarry):
                if RunModes["codingquarry"] == "rna-bam":
                    ...
                    if StartWeights["codingquarry"] > 0:
                        checkQuarry = lib.runCodingQuarryTrained(
                            MaskGenome,
                            aug_species,
                            os.path.join(args.out, "predict_misc", "CodingQuarry"),
                            args.cpus,
                            Quarry,
                        )
                        if not checkQuarry:
        ...
        if Quarry:
            ...
            if os.path.isdir(
                os.path.join(args.out, "predict_misc", "CodingQuarry", "ParameterFiles")
            ):
                lib.copyDirectory(
                    os.path.join(
                        args.out, "predict_misc", "CodingQuarry", "ParameterFiles"
                    ),
                    os.path.join(LOCALPARAMETERS, "codingquarry"),
                )
        else:
        ...
        if "snap" in RunModes:
            if not lib.checkannotations(SnapPredictions):
                if (
                    ...
                    if lib.snap_run_check(
                        SnapPredictions, FinalTrainingModels, StartWeights["snap"]
                    ):
                        lib.log.info(
                            "Running SNAP gene prediction, using training data: {:}".format(
                                FinalTrainingModels
                            )
                        )
                        lib.runSnap(
            ...
            if RunModes["snap"] != "pretrained":
                if SNAP and os.path.isfile(
                    os.path.join(args.out, "predict_misc", "snap-trained.hmm")
                ):
                    shutil.copyfile(
                        os.path.join(args.out, "predict_misc", "snap-trained.hmm"),
                        os.path.join(LOCALPARAMETERS, aug_species + ".snap.hmm"),
        ...
        if "glimmerhmm" in RunModes:
            if not lib.checkannotations(GlimmerPredictions):
                if (
                    RunModes["glimmerhmm"] == "pretrained"
                    and StartWeights["glimmerhmm"] > 0
                    and os.path.isdir(os.path.join(LOCALPARAMETERS, "glimmerhmm"))
                ):
                    lib.log.info(
                        "Running GlimmerHMM gene prediction, using pretrained HMM profile"
                    )
                    lib.runGlimmerHMMTrained(
                        MaskGenome,
                        os.path.join(LOCALPARAMETERS, "glimmerhmm"),
                        os.path.join(args.out, "predict_misc"),
                        GlimmerPredictions,
                    ...
                    if lib.glimmer_run_check(
                        GlimmerPredictions,
                        FinalTrainingModels,
                        StartWeights["glimmerhmm"],
                    ):
                        lib.log.info(
                            "Running GlimmerHMM gene prediction, using training data: {:}".format(
                                FinalTrainingModels
                            )
                        )
                        lib.runGlimmerHMM(
                ...
                lib.log.info(
                    "Existing GlimmerHMM predictions found: {:}".format(
                        GlimmerPredictions
                    )
                )
            ...
            if RunModes["glimmerhmm"] != "pretrained":
                if GLIMMER and os.path.isdir(
                    os.path.join(args.out, "predict_misc", "glimmerhmm")
                ):
                    lib.copyDirectory(
                        os.path.join(args.out, "predict_misc", "glimmerhmm"),
                        os.path.join(LOCALPARAMETERS, "glimmerhmm"),
    ...
    if args.keep_evm and os.path.isfile(EVM_out):
        ...
        evm_cmd = [
            ...
            MaskGenome,
            "-l",
            os.path.join(args.out, "logfiles", "funannotate-EVM.log"),
            "-m",
            str(args.min_intronlen),
            "-i",
    ...
    lib.log.info(
        "now filtering out bad gene models (< %i aa in length, transposable elements, etc)."
        % args.min_protlen
    )
    ...
    if args.trnascan:
        ...
        trna_result = lib.runtRNAscan(
            MaskGenome,
            os.path.join(args.out, "predict_misc"),
            tRNAscan,
            cpus=args.cpus,
            precalc=args.trnascan,
        )
    else:
    ...
    final_stats = os.path.join(
        args.out, "predict_results", organism_name + ".stats.json"
    )
    ...
    discrep = os.path.join(
        args.out, "predict_results", organism_name + ".discrepency.report.txt"
    )
    ...
    lib.log.info(
        "Funannotate predict is finished, output files are in the %s/predict_results folder"
        % (args.out)
    )
    ...
    if (
        args.rna_bam
        and args.pasa_gff
        and os.path.isdir(os.path.join(args.out, "training"))
    ):
        lib.log.info(
  ...
  funannotate update -i {:} --cpus {:} \\\n\
        ...
        lib.log.info(
            "Your next step might be functional annotation, suggested commands:\n\
-------------------------------------------------------\n\
...
-------------------------------------------------------\n\
                ".format(
                args.out, args.cpus, args.out, args.out, args.cpus
            )
        )
    ...
    lib.log.info(
        "Training parameters file saved: {:}".format(
  ...
  funannotate species -s {:} -a {:}\n".format(
    ...
    if os.path.exists("discrepency.report.txt") and os.path.isfile(
        "discrepency.report.txt"
    ):
        os.rename(
            "discrepency.report.txt", os.path.join(gag3dir, "discrepency.report.txt")
        )
